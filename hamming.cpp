...
// October 13, 2016
// This code prompts for up to a fourteen-digit hexadecimal value
// then prints out the corresponding hamming code with the input value
// contained in the parity bits 
#include <iostream>
#include <climits>
#include <bitset>
#include <math.h>
...
// get number of bits used to hold a long, or the maximum number of bits 
...
// get the minimum number of bits need to express the integer val in binary (with no leading zeros)
// val: the value we want to express in binary 
// return: the minimum number of digits needed to express val in binary
unsigned int getMinBits(long val) {	
	if (val == 0) {
		return 1;
	}else {
		// find the index of the MSB in our long integer
		// loop invariant: there are at most i + 1 bits needed to express val
...
unsigned int getMinParityBits(unsigned int m) {
	...
	// (m + r + 1) <= 2^r
	// see "Structured Computer Organization" page 79 for this formula
	while((m + r + 1) > pow(2,r)) 
		r++;
	return r;
}
...
long hamming (long val) {		
	...
	int tot_bits = num_parity_bits + num_data_bits;
	int hamming_arr[tot_bits];
	// loop invariant: the first i elements of hamming_arr have been assigned to 0
	for ( int i = 0; i < tot_bits; i++)
		hamming_arr[i] = 0;
	// loop invariant: the first i parity bits have been initialized to -1 in hamming_arr
	...
	// loop invariant: the first val_i data bits have been read in from val into hamming_arr
	int i = 0;
	int val_i = 0;
	while(i < tot_bits) {
		if (hamming_arr[i] != -1){
	...
	// loop invariant: the first p parity bits have been calculated for even parity
	for(int p = 0; p < num_parity_bits; p++) {
		int curbit = pow(2,p);
		int p_sum = 0;
		for (int i = curbit; i <= tot_bits; i++) {
	...
	// loop invariant: the first i bits of hamming_arr have been added to the hamming code
	long hamming_code = 0;
	for(i = 0; i < tot_bits; i++) {
		hamming_code += hamming_arr[i] * pow(2,tot_bits - i - 1);
	}

	// print out the final hamming code in binary, preceded by a tab,
	// so that it lines up with the previously printed data bits
	// loop invariant: the first i bits of the hamming code have been printed
	cout << "hamming code:\t";
	for (int i = 0 ; i < tot_bits; i++){
		cout << hamming_arr[i];
	}
...
// prompt for up to a 14-digit hexadecimal number and print out the 
